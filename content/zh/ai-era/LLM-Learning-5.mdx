...
**前情提要**：上一篇 [拆开 KV cache 这个「反派」](./LLM-Learning-4.mdx) 咱们正面拆了 KV cache 的物理构成，并把所有架构创新分成了「算得快 / 经济学优化 / 硬件常数」三类。这一篇要看的「scale-up domain 加大」，本质上就是第三类——硬件常数（让物理墙整体后退）。但它解决的具体问题，跟你直觉里想的可能不一样。
