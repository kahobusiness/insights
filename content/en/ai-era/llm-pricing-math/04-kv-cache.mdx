...
**Recap**: in the previous post, [From Inference Latency to Inference Cost](./03-latency-to-cost.mdx), we switched to the operator's view and saw what the same equations say when the Y-axis becomes "cost per token." But the KV cache villain only ever appeared as symbols — never opened up. This post fixes that.
