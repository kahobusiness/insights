...
**Recap**: in the previous post, [How Inference Actually Works — Starting with "Moving Stuff vs. Computing Stuff"](./01-how-inference-works.mdx), we built the core intuition: inference time = move time + compute time, whichever is slower wins; batching amortizes the cost of moving parameters, but it cannot amortize the KV cache.
