...
Flip the Y-axis on the previous post's simulator from "latency" to "cost per token" and you get the chart below — three reference lines (hyperbola, KV, compute) plus a bold total-cost curve. Drag B small and watch the total cost spike; drag it toward the sweet spot and the curve visibly flattens; push B very large and the curve pins itself onto the compute floor.

<CostBatchSimulator lang="en" />
