...
active parameters: how many parameters actually participate in one generation step.
```

The value of MoE is that it can increase total parameter count, giving the model more capacity, while controlling the number of active parameters per token so inference cost does not grow linearly with total parameters.
...
1. Why can capability improvement be continuous while the user experience feels sudden?
2. Why does a larger model not automatically mean a more reliable product?
3. Why does MoE make us distinguish total parameters from active parameters?
