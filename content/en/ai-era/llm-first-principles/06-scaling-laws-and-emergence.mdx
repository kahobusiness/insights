...
training and prediction error tend to decrease along relatively smooth and predictable curves.
```

The key metric here is loss.
...
Lower loss means the model assigns higher probability to the true next token. It also means the model is modeling the language distribution more closely.
