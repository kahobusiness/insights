...
test-time compute: spend more computation during inference, not only during training.
```

This has two direct product implications.
...
1. Do temperature and top-p change model capability, or sampling behavior?
2. Why does the context window provide visibility, not long-term memory or guaranteed attention?
3. Why can reasoning models trade more test-time compute for better complex-task performance?
