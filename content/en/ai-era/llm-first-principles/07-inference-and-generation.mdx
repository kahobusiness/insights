...
For example, if top-p = 0.9, the system sorts tokens from high to low probability, then keeps only the candidates whose cumulative probability reaches 90%. This filters out the long tail of very unlikely and often unstable tokens.
...
### 1. Parsers: turn output into something machines can check

If the output must be structured JSON, it is not enough for it to "look like JSON."

The system should actually parse it:

```text
Can it be parsed?
Are required fields present?
Are field types correct?
Are array lengths valid?
Is there extra explanatory text outside the JSON object?
```

If parsing fails, the usual strategy is not to show broken JSON to the user. The system can retry automatically.

During retry, it can feed the parse error back to the model:

```text
The previous output was not valid JSON. Error: missing closing brace.
Return only JSON that matches the schema. Do not include explanation.
```

But retry needs care. If the original task lacks enough information, the system should not pressure the model to invent fields. It should allow null values, error codes, or a request for more user input.

### 2. Function Calling and Schema: narrow the output space

If the model needs to call a tool, it is better not to let it freely write:

```text
I want to look up order 12345.
```

Instead, ask it to produce a schema-constrained call:
...
The value of function calling is that the model does not directly execute the action. It first turns intent into structured parameters.

The system can then check whether those parameters match the tool signature, business rules, and permission model.

Schema can constrain:
...
how nested structures are shaped.
```

Schema does not guarantee the model is always correct, but it greatly reduces the space of free-form generation.

### 3. Validators: check business correctness beyond syntax

A parser only tells you whether the structure is valid.

A validator checks things like:

```text
Does the order ID actually exist?
Does the amount match the database?
Is the date within the allowed range?
Does the user have permission to perform this action?
Is the generated SQL read-only?
Do the cited sources actually support the conclusion?
```

These checks should usually be handled by deterministic programs, databases, rule engines, or tool results, not by asking the model "are you sure?"

Models are good at generating explanations. They are not reliable final auditors of their own output.

In high-risk workflows, validators should also decide what happens after failure:

```text
retry automatically;
fall back to human confirmation;
return a request for missing information;
refuse execution;
show a draft instead of submitting directly.
```

### 4. max_tokens and Post-processing: length control is not just truncation
...
### 5. Stop Sequences: brake at explicit boundaries
