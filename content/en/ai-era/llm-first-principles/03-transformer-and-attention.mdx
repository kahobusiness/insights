...
For a generative language model, it usually cannot peek at future tokens. Training and generation use a causal mask, so each position can only reference itself and positions that have already appeared before it.
...
Then the model compares the current token's Query with the Keys of other tokens. A higher match means the current token should reference that position more strongly. These raw matching scores pass through a softmax, turning them into attention weights that add up to 1. After those weights are computed, the model uses them to take a weighted sum of the other tokens' Values.
...
## 12. Technical Appendix: The Intuition Behind the Attention Formula

The most common way to write Self-Attention is:

```text
Attention(Q, K, V) = softmax(QK^T / sqrt(d_k))V
```

You can read this formula in four steps:

```text
Q: the current token asks what it should reference;
K: each context token provides a matchable label;
QK^T: the current question is compared with all labels;
softmax(...)V: similarities become weights, then those weights summarize the corresponding information.
```

Dividing by `sqrt(d_k)` keeps similarity scores from growing too large as vector dimensions increase. `softmax` turns those scores into attention weights that can be used for a weighted average.

So the intuition behind the formula is still the same sentence this post has been building toward:

> Which positions in the context should the current token take information from, and how much from each?
...
2. Why does information being visible in the context window not mean it will definitely be used?
3. Why do RAG, tool use, and prompt structure all help the model organize context?
