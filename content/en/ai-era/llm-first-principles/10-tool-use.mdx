...
Real APIs do not all use the same field names. Anthropic `tool_use` blocks use `name` / `input`; OpenAI Chat Completions use `function.name` / `function.arguments`; OpenAI Responses API uses `name` / `arguments`. To keep the mechanism readable, the examples below use a simplified `name` + `arguments` shape.
...
They are usually lower risk, but they are not risk-free. Tool results enter the model context. The model may treat returned content as fact, or even mistake malicious instructions inside retrieved content for instructions it should follow. Read tools still need permission, privacy, prompt-injection, and data-exfiltration controls.
...
1. Why must tool use go through schema, permissions, validation, and auditing instead of letting the model operate systems directly?
2. Why do read tools and write tools have fundamentally different risk?
3. What is the difference between Tool Use and an Agent?
