...
![Abstract language symbols split into tokens, mapped into embedding vectors, and passed into a neural network](/llm-first-principles/02-token-and-embedding/cover.png)
...
![Abstract characters and word fragments receive naive independent ids, then become fragmented and misaligned](/llm-first-principles/02-token-and-embedding/naive-numbering.png)
...
To make these characters computable, we need to turn them into numbers. Let's start with the simplest possible idea: give each character or word fragment its own independent id, such as `我=01`, `喜=02`, `欢=03`, `苹=04`, and `果=05`.
...
![A continuous abstract text stream split by a tokenizer into token cards of different widths](/llm-first-principles/02-token-and-embedding/tokenization.png)

The component that cuts text into tokens is called a tokenizer. Despite the Chinese translation sometimes sounding like "word segmentation," this is not the same as dictionary-based Chinese word segmentation such as jieba. Modern tokenizers often work with bytes, characters, or subword fragments. They may merge adjacent Chinese characters into one token, or split text more finely depending on the vocabulary.
...
![Tokens first become arbitrary ids before a lookup table maps them into embedding vectors](/llm-first-principles/02-token-and-embedding/token-id-lookup.png)
...
The id 9174 is not "larger" or "more fruit-like" than 103. It is just an index in the vocabulary. A hotel room number can point to a room, but the number itself does not contain the furniture, guest, or relationships inside that room.
...
![Colored tokens map to ids, then look up high-dimensional embedding vectors before flowing into a neural network](/llm-first-principles/02-token-and-embedding/id-to-embedding.png)
...
![Similar tokens cluster in a high-dimensional vector space while one token moves along context-dependent paths to different semantic regions](/llm-first-principles/02-token-and-embedding/embedding-space.png)
...
![High-dimensional vector space represents meaning through relative positions, clusters, and directional relationships](/llm-first-principles/02-token-and-embedding/vector-semantics.png)
...
![The same input can split into token streams of different lengths, affecting cost, latency, and context budget](/llm-first-principles/02-token-and-embedding/tokenization-product-impact.png)
...
![Common misconceptions about tokens and embeddings are filtered toward the correct view of vector relationships](/llm-first-principles/02-token-and-embedding/token-misconceptions.png)
...
![Language becomes model-computable numbers through text, tokens, ids, embeddings, contextual vectors, and neural computation](/llm-first-principles/02-token-and-embedding/language-to-numbers-summary.png)
...
## 10. Technical Appendix: Why Token IDs Do Not Directly Represent Meaning

A token id looks like a number, but it is more like a seat number in the vocabulary.

```text
token: apple
token id: 12345
embedding: [0.12, -0.03, 0.87, ...]
```

What enters continuous neural computation is not the number `12345` itself, but the vector looked up from the embedding table. Two numerically adjacent token ids do not mean the tokens are semantically close. Semantic closeness usually appears in the relationships among embeddings and later contextual representations in high-dimensional space.

So token id solves the problem of indexing the vocabulary. Embedding solves the problem of putting symbols into a learnable relationship space.

## 11. Three Questions You Should Be Able to Answer

After reading this post, try answering these in your own words:

1. What problems do tokens, token ids, and embeddings each solve?
2. Why is a token id an index rather than meaning itself?
3. Why can the same sentence have different cost and behavior across languages or tokenizers?
