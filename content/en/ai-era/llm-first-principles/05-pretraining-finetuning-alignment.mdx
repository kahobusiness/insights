...
Common methods include RLHF, RLAIF, and DPO. They share one first-principles idea: turn "which answer is better" into a training signal that updates the model.

At a high level, alignment often follows this chain:

```text
Collect or construct candidate responses for the same prompt.
Use humans, AI judges, or existing preference data to decide which is better.
...
The algorithmic details matter, though. RLHF and RLAIF usually train a reward model, then use reinforcement learning to optimize the policy. DPO directly optimizes the model on pairwise preference data; it does not require online candidate generation or a separately trained reward model.
...
![Common misunderstandings about pretraining, fine-tuning, and alignment are separated into different training layers](/llm-first-principles/05-pretraining-finetuning-alignment/alignment-misconceptions.png)
...
![Training layers plus context, retrieval, tools, safety, and evaluation systems jointly shape product capability](/llm-first-principles/05-pretraining-finetuning-alignment/training-product-engineering.png)
...
![A model moves from pretraining continuation through supervised fine-tuning and preference alignment into assistant behavior](/llm-first-principles/05-pretraining-finetuning-alignment/assistant-summary.png)
...
## 9. Three Questions You Should Be Able to Answer

After reading this post, try answering these in your own words:

1. What capability or behavior does pretraining, supervised fine-tuning, and preference alignment each shape?
2. Why should fine-tuning not be understood simply as "injecting new knowledge" into the model?
3. When an AI assistant behaves poorly, why should you first diagnose which layer the problem belongs to?
