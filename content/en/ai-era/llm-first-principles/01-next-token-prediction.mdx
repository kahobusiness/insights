...
This post does one thing: reduce LLMs to the most basic question behind the whole series — given context, why is "predicting the next token" enough to become the starting point for understanding LLMs?
...
There are many ways to explain "first principles." The idea can be traced back to Aristotle's discussion of first principles, but in today's engineering and product context, it is usually used as a method of decomposition:
...
1. Why is "predict the next token" not a shallow task?
2. Why is model generation a probabilistic path rather than a fixed answer retrieved from a database?
3. Why should modern AI products be evaluated by more than whether the model is "smart"?
