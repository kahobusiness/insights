...
![The model uses the true next token as the training target and updates parameters through prediction error feedback](/llm-first-principles/01-next-token-prediction/training-next-token.png)
...
![Many language traces are compressed into reusable structure, making prediction look like understanding at the output layer](/llm-first-principles/01-next-token-prediction/prediction-as-understanding.png)
...
![As model scale, data, and compute grow, the simple prediction task crosses increasingly complex capability thresholds](/llm-first-principles/01-next-token-prediction/scaling-bridge.png)
...
![A large language model summarized as probabilistic prediction, compressed world structure, and a surrounding product system](/llm-first-principles/01-next-token-prediction/llm-summary.png)
