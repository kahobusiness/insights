...
This is the first post in the "Understanding LLMs from First Principles" series, and it also serves as the guidepost for the whole series. In 2026, large language models (LLMs) have already become part of our work and everyday life. Across this series, we will use first principles thinking in plain language to analyze the most fundamental mechanisms of LLMs, then gradually expand toward everything built on top of them: tokens, token prediction, why models appear to develop "intelligence," pretraining, post-training, tool use, agents, engineering systems, and commercialization. I hope the series gives you something useful.
...
With the same curiosity, this series will use first principles thinking to look inside the mechanisms behind the model.

Before we talk about the "first principle" of large language models, let's begin with what "first principles" means.

There are many ways to explain "first principles." The version I find most useful goes back to Aristotle. Roughly speaking:
...
On the goal: my view is that, in order to further liberate productivity, humanity is pursuing artificial general intelligence (AGI): enabling machines to understand and handle open-ended human tasks. LLMs are an important branch of that pursuit, at least on the language side.

"Make machines understand and handle open-ended human tasks": answering questions, explaining concepts, writing code, summarizing articles, translating sentences, planning steps, and continuing a conversation based on context.
...
This is the key trait that distinguishes large language models from search engines, knowledge bases, databases, and expert systems. They are not built to store one fixed answer, but to learn the regularities of how context leads to what comes next in human language — so they can take on more general tasks and answer questions they have never seen before.

At the same time, this reveals the most critical training target of a large language model: given context, learn the conditional probability distribution of the next token.

Why is that enough to matter?

Because language naturally unfolds in sequence. When people write, speak, code, or derive a formula, they keep adding new content after what already exists. During training, the model learns what the next token could be under the current context and how likely each option is. During generation, the system samples from that distribution token by token, unfolding an open-ended problem into a complete answer step by step.
...
Now let's shrink the scale back down and return to "tokens."
...
What is a large language model actually trained to do? Its most basic training task is called next-token prediction.
