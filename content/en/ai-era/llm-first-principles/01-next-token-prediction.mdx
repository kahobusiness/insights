...
At the same time, this reveals the most critical element of a large language model: its core mechanism — autoregressive generation.
...
> At its core, the model is doing "token prediction."
...
```text
Given what has already been seen, predict the next small chunk;
