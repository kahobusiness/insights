...
In those cases, exact search can be more reliable than semantic similarity.
...
Was an edge case not verified?
```

### 3. Weaknesses of Grep-Style Search
...
```text
Why does the page sometimes jump back after login?
```

The code may not contain the phrase "jump back."
...
If the hypothesis is wrong, the search can drift.
...
```text
Where does this project implement permission isolation?
```

The relevant code may be spread across middleware, database schema, API guards, frontend routing, and tests. It may not literally say "permission isolation."
...
### 5. Complete Systems Often Mix Both
...
```text
how the model obtains the external evidence required by the current task.
```

RAG is one way to obtain evidence.
...
Does the result need to be cited back to the user?
```

If the evidence is stable documentation, RAG fits well.
