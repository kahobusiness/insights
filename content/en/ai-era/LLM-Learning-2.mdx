...
**Recap**: in the previous post, [How Inference Works, Inference Speed, and Inference Cost — Starting with "Moving Stuff vs. Computing Stuff"](./LLM-Learning-1.mdx), we built the core intuition — inference time = move time + compute time, whichever is slower wins; batching amortizes the cost of moving parameters but cannot amortize KV cache. In this post we'll pick up the pen and **translate those intuitions into equations**, to see exactly what GPU engineers draw on whiteboards every day.
